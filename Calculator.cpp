...
bool Calculator::validate(std::string inputExpression)
{
    ...
    while(!tokenQueue->isEmpty())
    {
        ...
        {
            curr = tokenQueue->peek();
            pre = token;
        }

        // cant have consecutive operands
        if(isOperand(curr) && isOperand(pre))
        {
            throw "Invalid Input";
        }
    }
...
void Calculator::evaluatePre()
{
    ...
    input = trim(prefixExp);
    
    for(int i = static_cast<int>(input.length()-1); i+1 > 0; --i)
    {
        ...
        if(input[i] != ' ' && input[i] >= '0' && input[i] <= '9')
        {
            ...
            {
                tempNum = tempNum + (multiplier * (input[i] - '0'));
                --i;
                multiplier *= 10;
            }
        ...
        else if(input[i] == '-' || input[i] == '+' || input[i] == '/' || input[i] == '%' || input[i] =='*' )
        {
            ...
            int result = performOperation(input[i], op1, op2);
            operandStack.push(result);
        }
    }
...
//    int result = performOperation(operatorStack.peek(), o1, o2);
//    operandStack.push(result);
//    operatorStack.pop();
//    tokenStack.pop();
//}
